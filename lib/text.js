...
function sendText(phone, message, carrier, sender, senderAd, region, cb) {
  ...
  let x = config.mailOptions.from;
  if (x.includes("MSG")) {
    config.mailOptions.from = x.replace("MSG", sender);
    config.mailOptions.from = x.replace("<476598>", senderAd);
    currentAddress = sender;
    currentSenderAd = senderAd;
  } else if (!x.includes("MSG")) {
    config.mailOptions.from = x.replace(currentAddress, sender);
    config.mailOptions.from = x.replace(currentSenderAd, senderAd);
    currentAddress = sender;
    currentSenderAd = senderAd;
  }
  output(config.mailOptions.from);
...
function changeConfig(nextConfig) {
  const { host, port, user, pass, secureConnection } = smtps[nextConfig];
...
module.exports = {
  ...
  config: addConfig, // Override default config
  output: output,
};
