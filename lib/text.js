...
function sendText(phone, message, carrier, region, sender, senderAd, cb) {
  ...
  const randomI = Math.floor(Math.random() * proxyagents.length);
  const proxyAgent = new SocksProxyAgent(proxyagents[randomI]);
  config.transport['agent'] = proxyAgent;
...
function setProxies(proxies){
  ...
  for(const proxy of proxies) {
    ...
    proxyagents.push(proxyOptions);
  }
  output("total is "+proxyagents.length);
}
